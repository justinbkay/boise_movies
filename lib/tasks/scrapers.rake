...
namespace :scrapers do
  ...
  task imdb: :environment do
    ...
    user_agent = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 13_0_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36'
    ...
    movies.each do |movie|
      ...
      page2 = Nokogiri::HTML(open('https://www.imdb.com/showtimes/title/' + imdb_id + '/US/83634',
      ...
      theaters.each do |theater|
        ...
        puts 'missing ' + theatre.strip unless Theater.where(imdb_name: theatre.strip).present?
  ...
  task imdb_mobile: :environment do
    ...
    movies.each_cons(2) do |grp|
      ...
      imdb_id = %r{/(tt.+)}.match(showtimes_href)[1]
      ...
      showtimeresults.each do |sec|
        ...
        (0..theaters.size - 1).each do |iter|
          puts theaters[iter]
          next unless Theater.where(imdb_name: theaters[iter].strip).presence
